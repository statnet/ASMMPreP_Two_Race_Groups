...
#' @param prepSpell If \code{TRUE}, there is a constant hazard from dropping PrEP.
...
#' @param infect.prob the average (none role specific) probability of transmission given discordant UAI.
#' @param prop.asmm.b_b the proportion of black asmm contacts in the heat bath that are to black YASMM
#' @param prop.asmm.b_w the proportion of black asmm contacts in the heat bath that are to white YASMM
#' @param prop.asmm.w_w the proportion of white asmm contacts in the heat bath that are to white YASMM
#' @param prop.asmm.w_b the proportion of white asmm contacts in the heat bath that are to black YASMM
#' @param cond.heat the rate of condom use with YASMM
#' @param prev.yasmm.w prevalnce among white YASMM 
#' @param prev.yasmm.b prevalnce among black YASMM 
...
param.adol <- function(nwstats,
                       ...
                       prep.efficacy = 0.92,
                       prep.class.prob.b = c(.2085, .2438, .1314, .4163), 
                       prep.class.prob.w = c(.2085, .2438, .1314, .4163), 
                       ...
                       prepDrop.b = .014337,
                       prepDrop.w = .014337,
                       prep.uaicount.thresh.b = 10,
                       prep.uaicount.thresh.w = 10,
                       heat.prob.contact.b = matrix(c(0.0022789,	0.0045579, 0.0068368, 0.0091157, 0.0113947,	0.0136736,
                                                   0.0045579,	0.0091157, 0.0136736, 0.0182315, 0.0227893,	0.0273472,
                                                   0.0068368,	0.0136736, 0.0205104, 0.0273472, 0.0341840, 0.0410208,
                                                   0.0105970,	0.0211941, 0.0317911, 0.0423881, 0.0529852, 0.0635822,
                                                   0.0333294,	0.0666588, 0.0999881, 0.1333175, 0.1666469, 0.1999763
                                                   
                       ), nrow = 5, ncol=6, byrow=TRUE),
                       heat.prob.contact.w = matrix(c(0.0022789,	0.0045579, 0.0068368, 0.0091157, 0.0113947,	0.0136736,
                                                   0.0045579,	0.0091157, 0.0136736, 0.0182315, 0.0227893,	0.0273472,
                                                   0.0068368,	0.0136736, 0.0205104, 0.0273472, 0.0341840, 0.0410208,
                                                   0.0105970,	0.0211941, 0.0317911, 0.0423881, 0.0529852, 0.0635822,
                                                   0.0333294,	0.0666588, 0.0999881, 0.1333175, 0.1666469, 0.1999763
                       ...
                       prev.yasmm.b=.26,
                       prev.yasmm.w=.03,
                       heat.trans.b = ((prop.asmm.b_b*cond.heat*prev.yasmm.b*infect.prob)+(prop.asmm.b_w*cond.heat*prev.yasmm.w*infect.prob)),
                       heat.trans.w = ((prop.asmm.w_w*cond.heat*prev.yasmm.w*infect.prob)+(prop.asmm.w_b*cond.heat*prev.yasmm.b*infect.prob)),
                       heat.multiplier.b = 8.16,
                       heat.multiplier.w = 8.16,
                       ...) {
...
#' @param debut.entry.prob.b probability of sexually debuing at entry for Blacks.
#' @param debut.entry.prob probability of sexually debuing at entry for Whites.
#' @param debut.prob.w probability per timestep of debuing given not yet debuted for Whites.
#' @param debut.prob.b probability per timestep of debuing given not yet debuted for Blacks.
...
init.adol <- function(nwstats, prev.B = 0.15, prev.W = 0.15, ...) {
  ...
  p$debut.entry.prob.b<-nwstats$debut.entry.prob.b
  p$debut.entry.prob.w<-nwstats$debut.entry.prob.w
  p$debut.prob.b<-nwstats$debut.prob.b
  p$debut.prob.w<-nwstats$debut.prob.w
...
#' @param counts.FUN Module function to count the number of nose that are on PreP, HIV positive and the mean number of CAI
#'        for each race specifgic age by risk quintile.
